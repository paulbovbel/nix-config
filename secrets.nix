let
  pbovbel = "ssh-rsa  paul@bovbel.com";
  white_tower = "age1wk8sq7rwy46a4rms25gwyvxjd0utt53l8nvwnuaujqmqj6ek5ujqjsgt8t";
in {
  "secrets/common/pbovbel-id_rsa.age".publicKeys = [pbovbel white_tower];
  "secrets/laptop/tailscale-oauth-authkey.age".publicKeys = [pbovbel white_tower];
  "secrets/server/tailscale-oauth-authkey.age".publicKeys = [pbovbel white_tower];
  "secrets/common/cachix-auth-token.age".publicKeys = [pbovbel white_tower];
  "secrets/common/pbovbel-password-hash.age".publicKeys = [pbovbel white_tower];
  "secrets/common/rbovbel-password-hash.age".publicKeys = [pbovbel white_tower];
}
